...
client.prototype.makeRegex=function()
{
	...
	for(i=0;i<this.entries.length;i++)
	{
		...
		{
			var newobj=[];
			_.each(this.entries[i],function(e){
				var tmpb=new RegExp(e.reg);
				newobj.push({regex:e.reg,options:e.options,fields:e.fields,built:tmpb});
			});
		}
		else
		{
			...
			_.each(obj,function(o){
				_.each(tmp,function(t){
					...
					newobj.push({regex:o.regex+t.reg,options:o.options.concat(t.options),fields:o.fields.concat(t.fields),built:tmpb})
				})
			})
		}
		obj=newobj;
...
{
	var obj={};
	_.each(this.builtRegex,function(r){
