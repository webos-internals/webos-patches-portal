...
form.prototype.multiFile=function(name,title,label,num)
{
	num=num || 3
	var obj={"name":name,"title":title,"label":label || ""};
	obj.content="";
...
form.prototype.boolean=function(name,title,label)
{
	var obj={"name":name,"title":title,"label":label || ""};
	obj.content="<div>Yes: <input type='radio' name='"+this.name+"["+name+"][good]'/></div>"
	obj.content+="<div>No: <input type='radio' name='"+this.name+"["+name+"][bad]'/></div>"
}
