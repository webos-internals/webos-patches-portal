...
var client=gitWrapper.client=function(options,callback)
{
	...
	this.repo={};
	this.getRepo(this.path,function(err){
		if(err)
			callback(err)
		else
			callback(false);
...
client.prototype.getBranches=function(callback,all)
{
	if(this.repo.branches)
		{
		var obj=[];
		_.each(this.repo.branches,function(branch){
			if(all)
			obj.push(branch)
			else
...
client.prototype.updateBranchesTags=function(callback)
{
	var self=this;
	var obj=[];
	this.getRepo(null,function(err){
		...
		_.each(obj,function(e){
			...
			{
				var tmp={"commits":e.commits,"branch":e.branch};
				res.branches.push(tmp)
			}
			if(e.type && e.type=="tag")
			{
				var tmp={"commits":e.commits,"tag":e.tag};
				res.tags.push(tmp)
			}
		})
