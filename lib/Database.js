...
var database = master.database = function(options)
{
	...
	this.Revision=sequelize.define("Revision",{
		patch_file: Sequelize.TEXT,
		tweak_file: Sequelize.TEXT,
	...
	this.Tweak=sequelize.define("Tweak",{
		path:Sequelize.STRING
	})
	this.Maintainer=sequelize.define("Maintainer",{
		name:Sequelize.STRING,
		email:Sequelize.STRING,
		public:Sequelize.BOOLEAN
	})
	this.models=["Patch","User","Permission","Role","Revision","File","Device","Version","Screenshot","Session"]
	...
	this.Revision.hasMany("tweaks")//,this.File,"revisions")
...
{
	var self=this;
	sequelize.drop(function(tables,error){
		if(error)
		console.log(error)
		if(callback)
			callback(self)
